...
@Component({
  selector: 'parking-sub-form-select',
...
export class SubFormSelectComponent {
  @Input() icon!: string;
  @Input() label!: string;
  ...
  get optionSelectedLabel(): string | undefined {
    return this.options?.find(
      (option) => option?.id === this.form.formGroup.value.value
    )?.label;
  }

  get optionsList(): OptionModel[] {
    const options = this.optionsListValue?.length
  ...
  private filterOptions(term: string): void {
    ...
    this.optionsListValue = filteredOptions.length
      ? filteredOptions
      : this.options;
